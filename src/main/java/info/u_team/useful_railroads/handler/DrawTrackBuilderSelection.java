...
public class DrawTrackBuilderSelection {
	...
	public static void onBlockHighlight(HighlightBlock event) {
		...
		TrackBuilderManager.create(rayTraceResult.getPos(), rayTraceResult.getFace(), player.world,
				new Vec3d(event.getInfo().getViewVector()), mode, doubleTrack).ifPresent(manager -> {
					final int red;
					final int blue;
	...
	public static void drawSelectionBox(MatrixStack stack, Vec3d projectedView, Collection<BlockPos> posList, float red,
		...
		posList.forEach(pos -> {
			...
			WorldRenderer.drawShape(stack, builder, VoxelShapes.fullCube(), pos.getX() - projectedView.x,
					pos.getY() - projectedView.y, pos.getZ() - projectedView.z, red, green, blue, alpha);
			buffer.finish();
		});
	}
	...
	public static void drawSleeperTool(final HighlightBlock event) {
		final BlockRayTraceResult rayTraceResult = (BlockRayTraceResult) event.getTarget();
		final BlockPos position = rayTraceResult.getPos();
		final BlockState state = Minecraft.getInstance().world.getBlockState(position);
		
		final MatrixStack matrixStack = event.getMatrix();
		final Vec3d projectedView = event.getInfo().getProjectedView();

		if(state.getBlock().equals(UsefulRailroadsBlocks.TRACK_BLOCK)) {
			drawSelectionBox(matrixStack, projectedView, Lists.newArrayList(position), 0, 1, 0, 1);
			return;
		}
				
		final BlockPos pos = position.up();
		final PlayerEntity player = Minecraft.getInstance().player;
